...
def create_liquid_handler_loader(num_blocks=None):
    def loader(num_blocks):
        from continual_rl.envs.liquid_handler import LiquidHandler
        grid_size = [8, 8]
        # grid_size = [6, 6]
        # num_blocks = [15, 15, 15] if num_blocks is None else num_blocks
        num_blocks = [4, 4] if num_blocks is None else num_blocks
        # num_blocks = [5, 5, 5, 5] if num_blocks is None else num_blocks
        env_spec = lambda: LiquidHandler(grid_size=grid_size, num_blocks=num_blocks, penalize_dist=True, full_reward=True)
        env_spec_eval = lambda: LiquidHandler(grid_size=grid_size, num_blocks=num_blocks, penalize_dist=True, full_reward=True)
        env_spec_eval_no_dist = lambda: LiquidHandler(grid_size=grid_size, num_blocks=num_blocks, penalize_dist=False, full_reward=True)
        # env_spec_eval_exp = lambda: LiquidHandler(grid_size=grid_size, num_blocks=num_blocks, penalize_dist=True, full_reward=False, fixed_experiment=0)
        env_spec_eval_exp = lambda: LiquidHandler(grid_size=grid_size, num_blocks=num_blocks, penalize_dist=True, full_reward=True)

        # env_spec_eval55 = lambda: LiquidHandler(grid_size=grid_size, num_blocks=[5, 5], penalize_dist=False, full_reward=True)
        # env_spec_eval33 = lambda: LiquidHandler(grid_size=grid_size, num_blocks=[3, 3], penalize_dist=False, full_reward=True)
        # env_spec_eval35 = lambda: LiquidHandler(grid_size=grid_size, num_blocks=[3, 5], penalize_dist=False, full_reward=True)
        # env_spec_eval26 = lambda: LiquidHandler(grid_size=grid_size, num_blocks=[2, 6], penalize_dist=False, full_reward=True)
        env_spec_eval_exp1 = lambda: LiquidHandler(grid_size=grid_size, num_blocks=num_blocks, penalize_dist=False, fixed_experiment=1)
        env_spec_eval_exp2 = lambda: LiquidHandler(grid_size=grid_size, num_blocks=num_blocks, penalize_dist=False, fixed_experiment=2)
        env_spec_eval_exp3 = lambda: LiquidHandler(grid_size=grid_size, num_blocks=num_blocks, penalize_dist=False, fixed_experiment=3)
        env_spec_eval_exp4 = lambda: LiquidHandler(grid_size=grid_size, num_blocks=num_blocks, penalize_dist=False, fixed_experiment=4)
        env_spec_eval_exp5 = lambda: LiquidHandler(grid_size=grid_size, num_blocks=num_blocks, penalize_dist=False, fixed_experiment=5)
        env_spec_eval_exp6 = lambda: LiquidHandler(grid_size=grid_size, num_blocks=num_blocks, penalize_dist=False, fixed_experiment=6)

        tasks = [
            StateTask(task_id=0, action_space_id=0, env_spec=env_spec, num_timesteps=100e6, time_batch_size=1,
            # StateTask(task_id=0, action_space_id=0, env_spec=env_spec, num_timesteps=0, time_batch_size=1,
                      eval_mode=False),
            StateTask(task_id=1, action_space_id=0, env_spec=env_spec_eval, num_timesteps=1e1, time_batch_size=1,
                      eval_mode=True, continual_eval_num_returns=10),
            StateTask(task_id=2, action_space_id=0, env_spec=env_spec_eval_no_dist, num_timesteps=1e1, time_batch_size=1,
                      eval_mode=True, continual_eval_num_returns=10),



            # StateTask(task_id=3, action_space_id=0, env_spec=env_spec_eval_exp, num_timesteps=1e1, time_batch_size=1,
            #           eval_mode=True, continual_eval_num_returns=1000),


            # StateTask(task_id=2, action_space_id=0, env_spec=env_spec_eval_exp6, num_timesteps=1e1, time_batch_size=1,
            #           eval_mode=True),
            # StateTask(task_id=2, action_space_id=0, env_spec=env_spec_eval55, num_timesteps=1e1, time_batch_size=1,
            #           eval_mode=True),
            # StateTask(task_id=3, action_space_id=0, env_spec=env_spec_eval33, num_timesteps=1e1, time_batch_size=1,
            #           eval_mode=True),
            # StateTask(task_id=4, action_space_id=0, env_spec=env_spec_eval35, num_timesteps=1e1, time_batch_size=1,
            #           eval_mode=True),
            # StateTask(task_id=5, action_space_id=0, env_spec=env_spec_eval26, num_timesteps=1e1, time_batch_size=1,
            #           eval_mode=True),
            # StateTask(task_id=2, action_space_id=0, env_spec=env_spec_eval_exp1, num_timesteps=1e1, time_batch_size=1,
            #           eval_mode=True),
            # StateTask(task_id=3, action_space_id=0, env_spec=env_spec_eval_exp2, num_timesteps=1e1, time_batch_size=1,
            #           eval_mode=True),
            # StateTask(task_id=4, action_space_id=0, env_spec=env_spec_eval_exp3, num_timesteps=1e1, time_batch_size=1,
            #           eval_mode=True),
            # StateTask(task_id=5, action_space_id=0, env_spec=env_spec_eval_exp4, num_timesteps=1e1, time_batch_size=1,
            #           eval_mode=True),
            # StateTask(task_id=6, action_space_id=0, env_spec=env_spec_eval_exp5, num_timesteps=1e1, time_batch_size=1,
            #           eval_mode=True),
        ]
        return Experiment(tasks, continual_testing_freq=1e4)

    return lambda num_blocks=num_blocks: loader(num_blocks)
...
def get_available_experiments():
    ...
    experiments = LazyDict({
        ...
        "liquid_handler_3x3": create_liquid_handler_loader(num_blocks=[3, 3])
