...
class Monobeast():
    ...
    @classmethod
    def _add_to_collected_stats(cls, stats, collected_stats):
            ...
            try:
                ...
                while True:
                    ...
                    with lock:
                        ...
                        collected_stats = self._add_to_collected_stats(stats, collected_stats)
            except Exception as e:
    ...
    def _collect_test_episode(pickled_args):
        ...
        while not done:
            ...
            if done:
                ...
                logs_to_return["log_eval_final_step"] = [step]
                logger.info(
        ...
        env.close()
        return step, returns, logs_to_return
    ...
    def test(self, task_flags, num_episodes: int = 10):
        ...
        for batch_start_id in range(0, num_episodes, self._model_flags.eval_episode_num_parallel):
            ...
            with Pool(processes=batch_num_episodes) as pool:
                ...
                for async_obj in async_objs:
                    episode_step, episode_returns, episode_logs_to_return = async_obj.get()
                    step += episode_step
                    returns.extend(episode_returns)

                    self._add_to_collected_stats(episode_logs_to_return, logs_to_return)
        ...
        for key, log in logs_to_return.items():
            if isinstance(log, list):
                numpy_log = np.array(log)
                stats[f"{key}_mean"] = numpy_log.mean()
                stats[f"{key}_std"] = numpy_log.std()
            else:  # TODO: just assuming the log is a valid logging type -- handle numpy arrays
                stats[key] = log
